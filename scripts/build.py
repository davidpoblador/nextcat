...
def build() -> None:
    ...
    # Appendix: changelog (only if CHANGELOG.md exists, generated by release-please)
    if CHANGELOG_FILE.exists():
        changelog_title = escape_typst(changelog["title"])
        appendix_title = escape_typst(strings.get("appendix", {}).get("title", "Annexos"))
        parts.append(f'#{{ set heading(numbering: "A.1.")')
        parts.append(f'   counter(heading).update(0)')
        parts.append(f'   heading(level: 1)[{appendix_title}: {changelog_title}]')
        parts.append(f'   render(read("../CHANGELOG.md"), h1-level: 2)')
        parts.append("}")
        parts.append("")
