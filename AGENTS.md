...
- `src/xarter/build.py` -- Typer CLI entrypoint (`uv run xarter`)
- `src/xarter/generate.py` -- Document generation logic (typst, index, changelog)
...
`just pdf` runs `uv run xarter` (generates `.typ` files in `build/`) then compiles them with `typst`. The intermediate `.typ` file is deleted after compilation. Output: `build/xarter-{version}.{lang}.pdf`.

`just site` builds the MkDocs website (`uv run --group docs mkdocs build`). The build script generates `mkdocs.yml` (from template + nav) and `xarter/index.md` (from chapters + strings).
...
- **Chapters** (`xarter/NN-*.md`): Automatically picked up by both PDF and website. No extra work needed.
- **Front matter** (`xarter/00-*.md`): Automatically picked up by both.
...
- **Excluded files**: `index.md`, `license.md`, and `about-author.md` in `xarter/` are excluded from the chapter list. If you add a new non-chapter markdown file, add it to the `excluded` set in `build()`.
